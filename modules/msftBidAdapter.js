...
extANData.placement_id = Number(bidderParams.placement_id);
  ...
  const placementOk =
    isNumber(params.placement_id) ||
    (isStr(params.placement_id) && params.placement_id !== '' && !isNaN(Number(params.placement_id)));

  const memberOk =
    (isNumber(params.member) ||
      (isStr(params.member) && params.member !== '' && !isNaN(Number(params.member)))) &&
    isNotEmptyString(params.inv_code);

  return !!(placementOk || memberOk);
},
